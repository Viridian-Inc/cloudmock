...
import { TableDescription, FilterCondition } from './types';
...
export function CodeGenerator({
  ...
  function generatePython(): string {
    if (mode === 'scan') {
      ...
      code += kwargs.length > 0 ? `result = table.scan(${kwargs.join(', ')})\n` : `result = table.scan()\n`;
  ...
  function generateGo(): string {
    ...
    if (activeIndex.sk && skValue) {
      if (skOp === 'begins_with') {
        ...
        keyExpr += ` AND ${activeIndex.sk} BETWEEN :sk AND :sk2`;
        const memberType = activeSkType === 'N' ? 'N' : 'S';
        exprValues.push(`\t\t\t":sk2": &types.AttributeValueMember${memberType}{Value: "${skValue2}"}`);
      } else {
        keyExpr += ` AND ${activeIndex.sk} ${skOp} :sk`;
      }
      const memberType = activeSkType === 'N' ? 'N' : 'S';
      exprValues.push(`\t\t\t":sk": &types.AttributeValueMember${memberType}{Value: "${skValue}"}`);
    }
  ...
  return (
    <div class="ddb-modal-backdrop" onClick={onClose}>
      <div class="ddb-modal lg" onClick={(e) => e.stopPropagation()}>
