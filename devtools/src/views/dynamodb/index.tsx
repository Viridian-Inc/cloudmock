...
import { AccessPatterns } from './access-patterns';
import { PartiQL } from './partiql';
import { Terminal } from './terminal';
import { DDBContext, ddbReducer, initialState, useHydrateState, usePersistState } from './store';
import './dynamodb.css';
...
function DynamoDBCore({ showToast }: CoreProps) {
  ...
  return (
    <div class="ddb-layout">
        ...
        {!selectedTable ? (
          <div class="ddb-empty-state">
            ...
            <div style="margin-top:16px;font-size:12px;color:var(--text-tertiary)">
              <kbd class="ddb-kbd">Ctrl+N</kbd> New Item
              &nbsp;&middot;&nbsp;
              <kbd class="ddb-kbd">Ctrl+F</kbd> Search
              &nbsp;&middot;&nbsp;
              <kbd class="ddb-kbd">Ctrl+R</kbd> Refresh
              &nbsp;&middot;&nbsp;
              <kbd class="ddb-kbd">Ctrl+T</kbd> Terminal
              &nbsp;&middot;&nbsp;
              <kbd class="ddb-kbd">Tab</kbd> Switch Tabs
            </div>
          </div>
        ) : (
          <div>
            ...
            <div class="ddb-tabs">
              {contentTabs.map(ct => (
                <button
                  key={ct.key}
                  class={`ddb-tab ${activeSubTab === ct.key ? 'active' : ''}`}
      ...
      {deleteConfirm && (
        <div class="ddb-modal-backdrop" onClick={() => setDeleteConfirm(null)}>
          <div class="ddb-modal" onClick={(e) => e.stopPropagation()}>
            <div class="ddb-modal-header">
              <h3>Delete Table</h3>
              <button class="btn-icon" onClick={() => setDeleteConfirm(null)}><XIcon /></button>
            </div>
      ...
      {truncateConfirm && (
        <div class="ddb-modal-backdrop" onClick={() => { if (!truncateProgress) setTruncateConfirm(null); }}>
          <div class="ddb-modal" onClick={(e) => e.stopPropagation()}>
            <div class="ddb-modal-header">
              <h3>Truncate Table</h3>
              <button class="btn-icon" onClick={() => { if (!truncateProgress) setTruncateConfirm(null); }}><XIcon /></button>
            </div>
            <div class="ddb-modal-body">
              {truncateProgress ? (
                <div>
      ...
      {deleteItemsConfirm && (
        <div class="ddb-modal-backdrop" onClick={() => setDeleteItemsConfirm(null)}>
          <div class="ddb-modal" onClick={(e) => e.stopPropagation()}>
            <div class="ddb-modal-header">
              <h3>Delete Items</h3>
              <button class="btn-icon" onClick={() => setDeleteItemsConfirm(null)}><XIcon /></button>
            </div>
            <div class="ddb-modal-body">
