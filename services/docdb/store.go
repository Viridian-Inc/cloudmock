...
func (s *Store) snapshotARN(id string) string {
	return fmt.Sprintf("arn:aws:rds:%s:%s:cluster-snapshot:%s", s.region, s.accountID, id)
}
...
func (s *Store) parameterGroupARN(name string) string {
	return fmt.Sprintf("arn:aws:rds:%s:%s:cluster-pg:%s", s.region, s.accountID, name)
}
