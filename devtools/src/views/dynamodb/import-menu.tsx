...
export function ImportMenu({ tableName, showToast, onComplete }: ImportMenuProps) {
  ...
  return (
    <>
      ...
      {showModal && (
        <div class="ddb-modal-backdrop" onClick={() => { if (!importing) { setShowModal(false); setPreview(null); setFileName(''); } }}>
          <div class="ddb-modal" onClick={(e) => e.stopPropagation()}>
            <div class="ddb-modal-header">
              <h3>Import Items</h3>
              <button class="btn-icon" onClick={() => { if (!importing) { setShowModal(false); setPreview(null); setFileName(''); } }}><XIcon /></button>
            </div>
